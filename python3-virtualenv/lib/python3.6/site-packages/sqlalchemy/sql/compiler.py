...
class SQLCompiler(Compiled):
    ...
    def _process_parameters_for_postcompile(
        ...
        * literal_binds compiled with the literal_execute flag.  Used for
        ...
        statement = re.sub(r"\[POSTCOMPILE_(\S+)\]", process_expanding, self.string)
        ...
        if is_new_cte:
            ...
            if not cte_pre_alias_name and cte not in self.ctes:
                ...
                if cte.recursive:
                    ...
                    recur_cols = [
                        c
                        for c in util.unique_list(col_source._all_selected_columns)
                        if c is not None
                    ]

                    text += "(%s)" % (
                        ", ".join(
                            self.preparer.format_column(ident, anon_map=self.anon_map)
    ...
    def _with_schema_translate(self, schema_translate_map):
        ...
        def symbol_getter(obj):
            ...
            if name in schema_translate_map and obj._use_schema_map:
                ...
                return quoted_name("[SCHEMA_%s]" % (name or "_none"), quote=False)
            else:
                return obj.schema
